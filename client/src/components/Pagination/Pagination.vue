...
import type { PageOptionsInterface } from "@/types";
...
<template>
  <div v-if="props.totalRecords > 0">
    <div class="pagination">
      <button v-if="state.page > 1" class="btn btn-primary" @click="changePage(0)">&lt;&lt;</button>

      <button v-if="state.page > 1" class="btn btn-primary" @click="changePage(-1)">&lt;</button>
      <span class="pagination__pages">
        {{ isMobile ? `${state.page}/${pages}` : `Page ${state.page} sur  ${pages}` }}
      </span>

      <button v-if="state.page < pages" class="btn btn-primary" @click="changePage(1)">></button>
      <button v-if="state.page < pages" class="btn btn-primary" @click="changePage(pages)">>></button>
