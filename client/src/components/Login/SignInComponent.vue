...
<template>
  <form class="sign-in" @submit.prevent="login">
    ...
    <div class="sign-in__subtitle-secondary" @click="switchComponent()" @keydown="switchComponent()">Créer un compte</div>
    <main class="sign-in__inputs">
      ...
      <div class="sign-in__form-items">
        <label for="email">
          <input id="password" v-model="state.password" class="sign-in__form-input" type="password" placeholder="Mot de passe" />
        </label>
      </div>
    </main>
    <div class="sign-in__form-items--error" v-if="props.status === 'error_login'">Adresse mail et/ou mot de passe invalide</div>
    <div class="sign-in__footer">
      <button id="login-button" type="submit" class="btn btn-primary font-600" :disabled="!validatedFields" :class="{ 'sign-in__button-disabled': !validatedFields }">
