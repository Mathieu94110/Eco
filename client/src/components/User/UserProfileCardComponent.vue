...
import type { newUserInfoInterface, UserInterface } from "@/shared/interfaces";

const state = reactive({
  newUserInfos: {} as UserInterface,
});

const props = defineProps<{ userInfos: UserInterface }>();
...
const onPickFile = (): void => {
  ...
  if (file && file[0]) {
    ...
    reader.onload = (e) => {
      state.newUserInfos.image = e.target ? e.target.result : "";
    };
  ...
  <div class="user-profile-card" v-if="state.newUserInfos">
    ...
    <div class="user-profile-card__body">
      <p class="user-profile-card__main-category" :class="{ active: isEditMode }">
        <span>Pseudo</span>
        <span v-if="isEditMode"><input data-test="userName" class="pl-5 font-600" v-model="userName" /> </span><span v-else>{{ userName }}</span>
      </p>
      <p class="user-profile-card__sub-categories" :class="{ active: isEditMode }">
        <span>Prénom</span>
        <span v-if="isEditMode"><input data-test="firstName" class="pl-5 font-600" v-model="firstName" /> </span><span v-else>{{ firstName }}</span>
      </p>
      <p class="user-profile-card__sub-categories" :class="{ active: isEditMode }">
        <span>Nom</span>
        <span v-if="isEditMode"><input data-test="lastName" class="pl-5 font-600" v-model="lastName" /> </span><span v-else>{{ lastName }}</span>
      </p>
      <p class="user-profile-card__sub-categories" :class="{ active: isEditMode }">
        <span>Email</span><span v-if="isEditMode"><input data-test="email" class="pl-5 font-600" v-model="email" /> </span><span v-else>{{ email }}</span>
      </p>
      <p class="user-profile-card__sub-categories" :class="{ active: isEditMode }">
        <span>Téléphone</span><span v-if="isEditMode"><input data-test="phone" class="pl-5 font-600" v-model="phone" /> </span><span v-else>{{ phone }}</span>
      </p>
      <p class="user-profile-card__sub-categories" :class="{ active: isEditMode }">
        <span>Adresse</span><span v-if="isEditMode"><input data-test="address" class="pl-5 font-600" v-model="address" /></span><span v-else>{{ address }}</span>
      </p>
      <p class="user-profile-card__sub-categories" :class="{ active: isEditMode }">
        <span>Code postal</span>
        <span v-if="isEditMode"><input data-test="zip" class="pl-5 font-600" v-model="zip" /></span>
        <span v-else>{{ zip }}</span>
      </p>
    </div>

    <div class="user-profile-card__footer">
      <span id="edit-profile" v-if="!isEditMode" @click="editProfileInfo()" @keydown="editProfileInfo()"> <i class="fa-solid fa-pen-to-square user-profile-card__edit-icon"></i></span>
