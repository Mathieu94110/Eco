...
import CardLayout from "../Layout/CardLayoutComponent.vue";
  ...
  <div class="edit-card">
    ...
    <CardLayout v-if="props.add">
      <template #image>
        ...
        <div v-else class="edit-card__imagePreviewed" :style="{ 'background-image': `url(${state.card.image ? state.card.image : props.add.image})` }"></div>
        <label for="image">
      ...
      <template #date>
        <span>{{ new Date(props.add.created_at as any).toLocaleDateString() }}</span>
      </template>
      <template #category>
        <label for="category">
